...
\begin{rmk}
In the complex plane~\(\C\), a number \(z\in\C\) induces an endomorphism through multiplication, and its conjugate~\(\conj{z}\) induces the adjoint endomorphism. Note \(\conj{\conj{z}}=z\) as in~(5.2.5.2), \(\conj{wz}=\conj{w}\ \conj{z}\) as in~(5.2.5.3), \(\conj{z}z=\abs{z}^2\) is the ``multiplicator'' of~\(z\) as in~(5.2.5.5), and \(z\)~is ``orthogonal'' if it is on the unit circle.
